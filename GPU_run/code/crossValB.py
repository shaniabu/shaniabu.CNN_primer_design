...
def get_InfoTotal(dataFolder):
	...
	data=np.array(outData, dtype=np.float32)
	...
	dataSorted=np.zeros((size[0],size[1]), dtype=np.float32)
	labelsSorted=np.zeros(size[0])
	sequencesSorted=[]
...
def get_Info(indexVar,dataFolder):
	...
	data = read_csv(dataFolder+'sequences.csv', header=None, names=["sequences"]).values.ravel()
	print('data set', data.shape)

	
	labels=read_csv(dataFolder+'labels.csv', header=None, names=["lables"]).values.ravel()
	...
	train=[] #will return the sequences with the corresponding index
	test=[]
	valid=[]
